...
public interface TcpSocketProperties extends SocketProperties {
   ...
   * <p>
   * Defaults to {@code true} even though Socket default is false because optimizing to reduce amount of network traffic over latency is
   * hardly ever a concern today.
   */
   ...
   * is transmitted correctly. Enabling this option with a non-zero Integer <I>clientSocketTimeout</I> means that a <B>close()</B> will
   * block pending the transmission and acknowledgement of all data written to the peer, at which point the socket is closed
   * <I>gracefully</I>. Upon reaching the linger clientSocketTimeout, the socket is closed <I>forcefully</I>, with a TCP RST. Enabling the
   * option with a clientSocketTimeout of zero does a forceful close immediately. If the specified clientSocketTimeout value exceeds 65,535
  ...
  /**
   * Will fail during sending if the remote host address cannot be resolved. However, it can be set to false to allow unresolved hosts (this
   * is useful on some circumstances like connecting through a proxy).
   */
