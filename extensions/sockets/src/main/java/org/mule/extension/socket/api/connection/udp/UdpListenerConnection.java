...
public class UdpListenerConnection extends AbstractUdpConnection implements ListenerConnection {
  ...
  /**
   * One worker is created per received package. If the other end of the connection is awaiting for a response, one will be sent but not
   * from the same listener socket the source has. The response will be sent from a new different {@link DatagramSocket} bound to a port
