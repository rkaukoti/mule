...
public class HttpRequesterProvider implements ConnectionProvider<HttpClient>, Initialisable {
   ...
   * going to be secured using TLS / SSL. If HTTPS was configured as protocol then the user can customize the tls/ssl configuration by
   * defining the tls:context child element of this listener-config. If not tls:context is defined then the default JVM certificates are
  ...
  /**
   * Reusable configuration element for outbound connections through a proxy. A proxy element must define a host name and a port attributes,
   * and optionally can define a username and a password.
   */
  @Parameter
  @Optional
  ...
  public HttpClient connect() throws ConnectionException {
    ...
    HttpClientConfiguration configuration =
        new HttpClientConfiguration.Builder().setUriParameters(new DefaultUriParameters(protocol, host, port))
