...
/**
 * Abstracts all the Jms Vendor specific configuration for the Jms integration test suite. An implementation of this class must be created
 * for each Jms Vendor that gets tested.
 *
 ...
 * {@link javax.jms.XAQueueConnectionFactory} and {@link javax.jms.XATopicConnectionFactory} available. These will be used to create JMS
 * {@link javax.jms.Connection} objects using the {@link #getConnection(boolean, boolean)} method of this class.
 *
 * Note that this class defines a single method for {@link #getMiddleDestinationName()} but the {@link AbstractJmsFunctionalTestCase} will
 * made available 'middle' destination references i.e. 'middle2' and 'middle3'.
 *
...
public interface JmsVendorConfiguration extends ParameterizedConfiguration {
  ...
  /**
   * The test middle queue name. For consistency this should always be 'middle'. This value is used to create multiple middle queues,
   * namely, 'middle', 'middle2', 'middle3'. You need to make sure that these queues are available in the the JMS provider being tested.
   *
  ...
  /**
   * The test broadcast topic name. For consistency this should always be 'broadcast'. Note that you need to make sure that this topic is
  ...
  /**
   * The protocol used for creating endpoints. This is usually 'jms' but for specific messaging transports such as WebsphereMQ the protocol
   * will be the protocol of the transport i.e. 'wmq'.
   *
  ...
  /**
   * @return a ConnectionFactory implementation used for unit testing of the provider, usually consisting of some mocked-up methods.
   */
