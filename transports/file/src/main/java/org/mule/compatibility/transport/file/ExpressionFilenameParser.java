...
/**
 * <code>ExpressionFilenameParser</code> can use any expression language supported by Mule to construct a file name for the current message.
 * Expressions can be xpath, xquery, mvel, header, function and more. For more information see
 * http://www.mulesoft.org/documentation/display/MULE3USER/Using+Expressions.
 * <p/>
 * For example an xpath expression can be defined to pull a message id out of an xml message and use that as the file name - <code>
 * #[xpath:/message/header/@id]
 * </code>
 * <p/>
 * This parser superseeds the (now removed) <code>org.mule.compatibility.transport.file.SimpleFilenameParser</code> which has been kept in
 * Mule 2 for compatibility. The following demonstrates how to achieve the same results when using the <code>ExpressionFilenameParser</code>
 * over the <code>SimpleFilenameParser</code>
 * <ul>
 ...
 * <li>#[COUNT] : #[org.mule.compatibility.transport.file.ExpressionFilenameParser.count()] - note that this is a global counter.</li>
