...
/**
 * Test case that verifies that multiple HTTPS connectors do not interfere with each other. The server endpoint uses an HTTP connector with
 * a custom certificate. The client endpoint has no key store configured, therefore it should not be able to connect to the server. However,
 * with the current behavior of the HTTPS transport, it will work. This is because HTTPS connectors set system properties with the key store
 * and trust store, that later on become the default for any SSL connection. The system property "mule.tls.disableSystemPropertiesMapping"
 * disables this behavior, and this test verifies that the two connectors will not interfere when this property is set.
 */
public class HttpsMultipleConnectorsTestCase extends FunctionalTestCase {
  ...
  protected void doSetUpBeforeMuleContextCreation() throws Exception {
    ...
    /*
     * The trust store might have been set by other test running in the same process, we need to clear it temporarily in order to run this
     * test correctly.
     */
