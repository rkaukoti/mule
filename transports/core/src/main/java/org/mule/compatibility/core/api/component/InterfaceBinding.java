...
public interface InterfaceBinding extends MessageProcessor {
  /**
   * This method is responsible for routing the Message via the MuleSession. The logic for this method will change for each type of router
   * depending on expected behaviour. For example, a MulticastingRouter might just iterate through the list of assoaciated endpoints sending
   * the message. Another type of router such as the ExceptionBasedRouter will hit the first endpoint, if it fails try the second, and so
   * on. Most router implementations will extends the FilteringOutboundRouter which implements all the common logic need for a router.
   *
   * @return a result message if any from the invocation. If the endpoint bound has a one-way exchange pattern configured a null result will
   *         always be returned.
