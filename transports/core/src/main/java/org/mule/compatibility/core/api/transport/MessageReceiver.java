...
/**
 * <code>MessageReceiver</code> is used to receive data from an external system. Typically an implementation of this interface will also
 * implement the listener interface for the external system. For example to listen to a JMS destination the developer would also implement
 * javax.jms.MessageListener. The endpoint (which creates the MessageReceiver) will then register the receiver with the JMS server. Where a
 * listener interface is not availiable the derived <code>MessageReceiver</code> will implement the code necessary to receive data from the
 * external system. For example, the file endpoint will poll a specified directory for its data.
 *
