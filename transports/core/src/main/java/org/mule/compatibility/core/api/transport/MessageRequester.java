...
/**
 * Combine {@link org.mule.compatibility.core.api.transport.MessageRequesting} with various lifecycle methods for the actual instances doing
 * message sending.
 *
...
public interface MessageRequester extends Connectable, MessageRequesting, LifecycleStateEnabled {
  /**
   * This method can perform necessary state updates before any of the {@link org.mule.compatibility.core.api.transport.MessageDispatching}
   * methods are invoked.
   *
  ...
  /**
   * After receiving a message, the dispatcher can use this method e.g. to clean up its internal state (if it has any) or return pooled
   * resources to whereever it got them during {@link #activate()}.
   *
