...
public class MessageAttachmentsTestCase extends AbstractELTestCase {
  ...
  public void assignValueToInboundAttachment() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addInboundAttachment("foo", dataHandler).build());
    assertUnsupportedOperation("message.inboundAttachments['foo']=new DataHandler('bar','text/plain')", event);
  }

  @Test
  public void assignValueToNewInboundAttachment() throws Exception {
    MuleEvent event = getTestEvent("");
    assertUnsupportedOperation("message.inboundAttachments['foo_new']=new DataHandler('bar','text/plain')", event);
  }
  ...
  public void inboundSize() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addInboundAttachment("foo", mock(DataHandler.class))
        .addInboundAttachment("bar", mock(DataHandler.class)).build());
  ...
  public void inboundContainsKey() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addInboundAttachment("foo", mock(DataHandler.class)).build());
  ...
  public void inboundEntrySet() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addInboundAttachment("foo", valA).addInboundAttachment("bar", valB).build());
    Set<Map.Entry<String, DataHandler>> entrySet =
        (Set<Entry<String, DataHandler>>) evaluate("message.inboundAttachments.entrySet()", event);
  ...
  public void inboundValues() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addInboundAttachment("foo", valA).addInboundAttachment("bar", valB).build());
    Collection<DataHandler> values = (Collection<DataHandler>) evaluate("message.inboundAttachments.values()", event);
  ...
  public void outboundClear() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addOutboundAttachment("foo", mock(DataHandler.class))
        .addOutboundAttachment("bar", mock(DataHandler.class)).build());
  ...
  public void outboundContainsValue() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).payload(valA).addOutboundAttachment("foo", valA).build());
  ...
  public void outboundEntrySet() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addOutboundAttachment("foo", valA).addOutboundAttachment("bar", valB).build());
    Set<Map.Entry<String, DataHandler>> entrySet =
        (Set<Entry<String, DataHandler>>) evaluate("message.outboundAttachments.entrySet()", event);
    assertEquals(2, entrySet.size());
    entrySet.contains(new DefaultMapEntry("foo", valA));
    entrySet.contains(new DefaultMapEntry("bar", valB));
  }
  ...
  public void outboundValues() throws Exception {
    ...
    event.setMessage(MuleMessage.builder(message).addOutboundAttachment("foo", valA).addOutboundAttachment("bar", valB).build());
    Collection<DataHandler> values = (Collection<DataHandler>) evaluate("message.outboundAttachments.values()", event);
    assertEquals(2, values.size());
    values.contains(valA);
    values.contains(valB);
  }
