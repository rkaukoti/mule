...
public final class HttpTransactionContext {
  ...
  private final CloseListener listener = new CloseListener<Closeable, CloseType>() {
    ...
    public void onClosed(Closeable closeable, CloseType type) throws IOException {
      if (isGracefullyFinishResponseOnClose()) {
        ...
        fc.fireEventUpstream(connection, new GracefulCloseEvent(HttpTransactionContext.this), null);
  ...
  // -------------------------------------------------------- Static methods
  static void bind(final HttpContext httpCtx, final HttpTransactionContext httpTxContext) {
    httpCtx.getCloseable().addCloseListener(httpTxContext.listener);
    REQUEST_STATE_ATTR.set(httpCtx, httpTxContext);
  }

  static void cleanupTransaction(final HttpContext httpCtx, final CompletionHandler<HttpTransactionContext> completionHandler) {
  ...
  static void cleanupTransaction(final HttpContext httpCtx, final HttpTransactionContext httpTxContext) {
    httpCtx.getCloseable().removeCloseListener(httpTxContext.listener);
    REQUEST_STATE_ATTR.remove(httpCtx);
  }

  static HttpTransactionContext currentTransaction(final HttpHeader httpHeader) {
  ...
  HttpTransactionContext cloneAndStartTransactionFor(final Connection connection) {
    return cloneAndStartTransactionFor(connection, ahcRequest);
  }

  HttpTransactionContext cloneAndStartTransactionFor(final Connection connection, final Request request) {
    final HttpTransactionContext newContext = startTransaction(connection, provider, request, future);
