...
/**
 * A Server managers is in charge to handle all ServerSocket connections and to handle incoming requests to an Execution Service for
 * processing.
 */
public interface HttpServerManager {
   ...
   * @return true if there's already a server created for that port and either the same host or an overlapping one (0.0.0.0 or any other if
   *         the serverAddress host is 0.0.0.0), false otherwise.
   */
  boolean containsServerFor(ServerAddress serverAddress);

  /**
   * @param serverAddress address of the server
   * @param workManagerSource work manager source to use for retrieving a {@link org.mule.runtime.core.api.context.WorkManager} for
   *        processing this server requests
