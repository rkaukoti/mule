...
/**
 * This constructs a nested map - keyed by "mapKey" - and then adds an entry in that named from the attribute "keyAttribute". Child elements
 * can then set a value (or values, if it is a collection) on {@link org.mule.runtime.config.spring.parsers.assembly.MapEntryCombiner#VALUE}
 */
public class ElementInNestedMapDefinitionParser extends AbstractSingleParentFamilyDefinitionParser {
  ...
  public ElementInNestedMapDefinitionParser(String mapSetter, String mapKey, String keyAttribute) {
    ...
    addDelegate(new ChildSingletonMapDefinitionParser(mapSetter)).registerPreProcessor(new AddAttribute(MapEntryCombiner.KEY, mapKey))
        .addCollection(mapSetter).setIgnoredDefault(true).removeIgnored(MapEntryCombiner.KEY)
