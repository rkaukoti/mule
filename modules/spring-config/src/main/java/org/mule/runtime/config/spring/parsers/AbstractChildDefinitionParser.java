 ...
 * {@link org.mule.runtime.config.spring.parsers.AbstractHierarchicalDefinitionParser} with methods that assume the data are associated with
 * a single property.
 *
 ...
 * {@link org.mule.runtime.config.spring.parsers.collection.ChildMapEntryDefinitionParser.KeyValuePair} it is assumed that a Map is being
 * processed and any child elements will be added to the parent Map. Similarly for
 * {@link org.mule.runtime.config.spring.parsers.collection.ChildListEntryDefinitionParser}.
 *
 * A single method needs to be overriden called {@link #getPropertyName} that determines the name of the property to set on the parent bean
 * with this bean. Note that the property name can be dynamically resolved depending on the parent element.
 *
  ...
  protected void postProcess(ParserContext context, BeanAssembler assembler, Element element) {
    ...
    if (null != propertyName) {
      ...
      if (!assembler.getBean().getBeanDefinition().isSingleton()) {
        ...
        assembler.insertSingletonBeanInTarget(propertyName, element.getAttribute(AbstractMuleBeanDefinitionParser.ATTRIBUTE_NAME));
      }
    }
  }
