...
public class CommonBeanDefinitionCreator extends BeanDefinitionCreator
{
    ...
    public CommonBeanDefinitionCreator()
    {
        try
        {
            ...
            this.beanDefinitionPostProcessor =
                    (BeanDefinitionPostProcessor) ClassUtils.getClass(Thread.currentThread().getContextClassLoader(),
                            TRANSPORT_BEAN_DEFINITION_POST_PROCESSOR_CLASS).newInstance();
        }
        catch (Exception e)
        {
            ...
            return cdm.getIdentifier().equals(ANNOTATIONS_ELEMENT_IDENTIFIER);
                      ...
                      .ifPresent(annotationsCdm ->
                      {
                          annotationsCdm.getInnerComponents().forEach(annotationCdm ->
                                  {
                                      previousAnnotations.put(
                                              new QName(from(annotationCdm).getNamespaceUri(), annotationCdm.getIdentifier().getName()),
