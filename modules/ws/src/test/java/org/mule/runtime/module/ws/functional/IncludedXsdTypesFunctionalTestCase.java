...
public class IncludedXsdTypesFunctionalTestCase extends AbstractWSConsumerFunctionalTestCase {
  ...
  /**
   * Verifies that a no parameter operation is successful having a WSDL definition file that imports the types from another WSDL file, which
   * includes the schema from yet another XSD file. If the metadata cannot be generated then the payload would be used as SOAP body and the
   * test would fail.
   */
  @Test
  public void metadataIsGeneratedAndPayloadIsIgnored() throws Exception {
    ...
    String expectedResponse =
        "<ns:noParamsResponse xmlns:ns=\"http://consumer.ws.module.runtime.mule.org/\">" + "<text>TEST</text></ns:noParamsResponse>";
