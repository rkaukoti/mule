 ...
 * built into the JRE 2. Xalan does not work with STAX 3. Having Xalan in the classpath overrides the default (STAX-compliant) factories for
 * transformations, validators, etc with ones that aren't STAX-compliant
 * <p/>
 * The result is that, while the ideal would be to implement this class by validating a STAXSource, that won't be possible until either we
 * can assume a JRE with bith STAX and a working Xalan fork, or there';s a xalan.jar that supports StAX.
 *
...
public class JsonSchemaXsdValidationFilter extends SchemaValidationFilter implements JsonSchemaFilter {
  ...
  public boolean accept(MuleEvent event) {
    ...
    try {
      ...
      String xmlString =
          (String) jToX.transform(msg.getPayload(), msg.getDataType().getMediaType().getCharset().orElse(getDefaultEncoding(muleContext)));
