...
/**
 * Base class for implementations of {@link ModelEnricher} which provides utility methods for enriching {@link BaseDeclaration declarations}
 * constructed from an annotated model
 *
...
public abstract class AbstractAnnotatedModelEnricher implements ModelEnricher {
   ...
   * <p/>
   * The annotated type is determined by querying the {@code declaration} for the {@link ImplementingTypeModelProperty} model property
   *
   * @param declaration a {@link BaseDeclaration} to be enriched
  ...
  protected <T> Class<T> extractExtensionType(BaseDeclaration<? extends BaseDeclaration> declaration) {
    return (Class<T>) declaration.getModelProperty(ImplementingTypeModelProperty.class).map(ImplementingTypeModelProperty::getType)
        .orElse(null);
  }
}
