...
/**
 * Implements simple connection management by using the {@link #before(OperationContext)} phase to set a connection as parameter value of
 * key {@link ExtensionProperties#CONNECTION_PARAM} into an {@link OperationContext}.
 *
 * @since 4.0
 */
public final class ConnectionInterceptor implements Interceptor {
  ...
  /**
   * Adds a {@code Connection} as a parameter in the {@code operationContext}, following the considerations in this type's javadoc.
   *
  ...
  private <T> ConnectionHandler<T> getTransactionlessConnectionHandler(OperationContext operationContext) throws ConnectionException {
    ...
    if (!connectionProvider.isPresent()) {
      throw new IllegalStateException(format(
          ...
          operationContext.getOperationModel().getName(), operationContext.getConfiguration().getModel().getExtensionModel().getName(),
  ...
  private <T extends TransactionalConnection> ConnectionHandler<T> getTransactedConnectionHandler(OperationContextAdapter operationContext,
      TransactionConfig transactionConfig) throws ConnectionException, TransactionException {
      ...
      try {
        if (currentTx.supports(txKey, txResource)) {
          ...
          throw new TransactionException(createStaticMessage(format(
              "Operation '%s' of extension '%s' is transactional but current transaction doesn't " + "support connections of type '%s'",
              operationContext.getOperationModel().getName(), operationContext.getConfiguration().getModel().getExtensionModel().getName(),
