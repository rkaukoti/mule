...
public interface ValueResolver<T> {
  ...
  /**
   * returns {@code false} if subsequent invocations to {@link #resolve(MuleEvent)} will return the same value. Notice that if it returns
   * {@code true}, then it might return different values per invocation but that's not guaranteed.
   *
