...
/**
 * Resolves a Component's Metadata by coordinating the several moving parts that are affected by the Metadata fetching process, so that such
 * pieces can remain decoupled.
 * <p/>
...
public class MetadataMediator {
  ...
  /**
   * Resolves the list of types available for the Content or Output of the associated {@link MetadataAware} Component, representing them as
   * a list of {@link MetadataKey}.
   * <p>
   * If no {@link MetadataKeyId} is present in the component's input parameters, then a {@link NullMetadataKey} is returned. Otherwise, the
   * {@link MetadataKeysResolver#getMetadataKeys} associated with the current Component will be invoked to obtain the keys
   *
   * @param context current {@link MetadataContext} that will be used by the {@link MetadataKeysResolver}
   * @return Successful {@link MetadataResult} if the keys are obtained without errors Failure {@link MetadataResult} when no Dynamic keys
   *         are a available or the retrieval fails for any reason
   */
  public MetadataResult<Set<MetadataKey>> getMetadataKeys(MetadataContext context) {
    ...
    try {
      ...
      final Set<MetadataKey> enrichedMetadataKeys = metadataKeys.stream()
          .map(metadataKey -> cloneAndEnrichMetadataKey(metadataKey, partOrder, 1)).map(MetadataKeyBuilder::build).collect(toSet());
  ...
  /**
   * Resolves the {@link ComponentMetadataDescriptor} for the associated {@link MetadataAware} Component using only the static types of the
   * Component parameters, attributes and output.
   *
   * @return a{@link MetadataResult} of {@link ComponentMetadataDescriptor} with the Static Metadata representation of the Component.
   */
  ...
  /**
   * Resolves the {@link ComponentMetadataDescriptor} for the associated {@link MetadataAware} Component using static and dynamic resolving
   * of the Component parameters, attributes and output.
   * <p>
   ...
   * @return Successful {@link MetadataResult} if the MetadataTypes are resolved without errors Failure {@link MetadataResult} when the
   *         Metadata retrieval of any element fails for any reason
   */
  ...
  /**
   * For each of the Component's {@link ParameterModel} creates the corresponding {@link TypeMetadataDescriptor} using only its static
   * {@link MetadataType} and ignoring if any parameter has a dynamic type.
   *
   * @return A {@link List} containing a {@link MetadataResult} of {@link TypeMetadataDescriptor} for each input parameter using only its
  ...
  /**
   * @return a {@link MetadataResult} of a {@link TypeMetadataDescriptor} representing the Component's content metadata based only on its
   *         static {@link MetadataType} type and ignoring if a {@link MetadataContentResolver} was available
   */
  ...
  /**
   * Creates a {@link TypeMetadataDescriptor} representing the Component's Content metadata using the {@link MetadataContentResolver}, if
   * one is available to resolve the {@link MetadataType}. If no the Component has no Content parameter, then {@link Optional#empty()} is
   * returned.
   *
   * @param context current {@link MetadataContext} that will be used by the {@link MetadataContentResolver}
   * @param key {@link MetadataKey} of the type which's structure has to be resolved
   * @return Success with an {@link Optional} {@link TypeMetadataDescriptor} representing the Component's Content metadata, resolved using
   *         the {@link MetadataContentResolver} if one is available to resolve its {@link MetadataType}, returning {@link Optional#empty()}
   *         if no Content parameter is present Failure if the dynamic resolution fails for any reason.
   */
  ...
  /**
   * @return a {@link MetadataResult} of {@link OutputMetadataDescriptor} representing the Component's output metadata based only on its
   *         static {@link MetadataType} and ignoring if a {@link MetadataOutputResolver} was available
   */
  ...
  /**
   * Creates an {@link OutputMetadataDescriptor} representing the Component's output metadata using the {@link MetadataOutputResolver}, if
   * one is available to resolve the output {@link MetadataType}.
   *
   * @param context current {@link MetadataContext} that will be used by the {@link MetadataContentResolver}
   * @param key {@link MetadataKey} of the type which's structure has to be resolved
   * @return Success with an {@link OutputMetadataDescriptor} representing the Component's output metadata, resolved using the
   *         {@link MetadataOutputResolver} if one is available to resolve its {@link MetadataType}. Failure if the dynamic resolution fails
   *         for any reason.
   */
  private MetadataResult<OutputMetadataDescriptor> getOutputMetadataDescriptor(MetadataContext context, MetadataKey key) {
    ...
    OutputMetadataDescriptor descriptor =
        outputDescriptor().withReturnType(outputMetadataResult.get()).withAttributesType(attributesMetadataResult.get()).build();
  ...
  /**
   * Given a {@link MetadataKey} of a type and a {@link MetadataContext}, resolves the {@link MetadataType} of the {@link Content} parameter
   * using the {@link MetadataContentResolver} associated to the current component.
   *
  ...
  /**
   * Given a {@link MetadataKey} of a type and a {@link MetadataContext}, resolves the {@link MetadataType} of the Components's output using
   ...
   * {@link MuleMessage#getAttributes()} using the {@link MetadataOutputResolver} associated to the current component.
   *
   * @param context {@link MetadataContext} of the Metadata resolution
   * @param key {@link MetadataKey} of the type which's structure has to be resolved
  ...
  /**
   * Uses the {@link MetadataDelegate} to resolve dynamic metadata of the component, executing internally one of the {@link MetadataType}
   * resolving components: {@link MetadataContentResolver#getContentMetadata} or {@link MetadataOutputResolver#getOutputMetadata}
   *
   ...
   * @return a {@link MetadataResult} with the {@link MetadataType} resolved by the delegate invocation. Success if the type has been
   *         successfully fetched, Failure otherwise.
   */
  ...
  /**
   * Given a {@link MetadataKey} instantiates the associated {@link MetadataKeyId} parameter this could be a simple {@link String} if the
   * key contains only one level or a complex type with {@link MetadataKeyPart} annotated fields for a multilevel key
   *
