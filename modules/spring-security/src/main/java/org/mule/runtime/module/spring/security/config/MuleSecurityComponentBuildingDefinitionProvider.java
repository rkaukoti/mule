...
/**
 * {@link org.mule.runtime.config.spring.dsl.api.ComponentBuildingDefinition} definitions for the components provided by the mule spring
 * security module.
 *
 * @since 4.0
 */
public class MuleSecurityComponentBuildingDefinitionProvider implements ComponentBuildingDefinitionProvider {
  ...
  public List<ComponentBuildingDefinition> getComponentBuildingDefinitions() {
    ...
    componentBuildingDefinitions
        .add(baseDefinition.copy().withIdentifier("delegate-security-provider").withTypeDefinition(fromType(SpringProviderAdapter.class))
            .withSetterParameterDefinition("name", fromSimpleParameter("name").build())
    ...
    componentBuildingDefinitions.add(baseDefinition.copy().withIdentifier("security-property")
        .withTypeDefinition(fromType(SecurityProperty.class)).withConstructorParameterDefinition(fromSimpleParameter("name").build())
    ...
    componentBuildingDefinitions
        .add(baseDefinition.copy().withIdentifier("authorization-filter").withTypeDefinition(fromType(AuthorizationFilter.class))
            .withSetterParameterDefinition("requiredAuthorities", fromSimpleParameter("requiredAuthorities",
