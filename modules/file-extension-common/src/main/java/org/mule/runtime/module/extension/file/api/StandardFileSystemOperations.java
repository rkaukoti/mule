...
public class StandardFileSystemOperations {
  ...
  /**
   * Obtains the content and metadata of a file at a given path. The operation itself returns a {@link MuleMessage} which payload is a
   * {@link InputStream} with the file's content, and the metadata is represent as a {@link FileAttributes} object that's placed as the
   * message {@link MuleMessage#getAttributes() attributes}.
   * <p>
   * If the {@code lock} parameter is set to {@code true}, then a file system level lock will be placed on the file until the input stream
   * this operation returns is closed or fully consumed. Because the lock is actually provided by the host file system, its behavior might
   * change depending on the mounted drive and the operation system on which mule is running. Take that into consideration before blindly
   * relying on this lock.
   * <p>
   ...
   * make an educated guess on the file's mime type. The user also has the chance to force the output enconding and mimeType through the
   * {@code outputEncoding} and {@code outputMimeType} optional parameters.
   *
   * @param config the config that is parameterizing this operation
  ...
  public OperationResult<InputStream, FileAttributes> read(@UseConfig FileConnectorConfig config, @Connection FileSystem fileSystem,
      MuleMessage message, @DisplayName("File Path") String path, @Optional(defaultValue = "false") boolean lock) {
    fileSystem.changeToBaseDir(config);
   ...
   * {@link FileAttributes#getPath()} will be used. If that's not the case, then an {@link IllegalArgumentException} will be thrown.
   * <p>
   ...
   * <p>
   * This operation also supports locking support depending on the value of the {@code lock} argument, but following the same rules and
   * considerations as described in the read operation.
   *
   ...
   * {@code createParentFolder} argument. If such argument is {@false}, then an {@link IllegalArgumentException} will be thrown.
   * <p>
  ...
  public void copy(@UseConfig FileConnectorConfig config, @Connection FileSystem fileSystem, @Optional String sourcePath, String targetPath,
      @Optional(defaultValue = "false") boolean overwrite, @Optional(defaultValue = "true") boolean createParentDirectories,
      MuleEvent event) {
    fileSystem.changeToBaseDir(config);
   ...
   * <p>
   * If {@code targetPath} doesn't exists, and neither does its parent, then an attempt will be made to create depending on the value of the
   * {@code createParentFolder} argument. If such argument is {@code false}, then an {@link IllegalArgumentException} will be thrown.
   * <p>
   * If the target file already exists, then it will be overwritten if the {@code overwrite} argument is {@code true}. Otherwise,
   * {@link IllegalArgumentException} will be thrown.
   * <p>
  ...
  public void move(@UseConfig FileConnectorConfig config, @Connection FileSystem fileSystem, @Optional String sourcePath, String targetPath,
      @Optional(defaultValue = "false") boolean overwrite, @Optional(defaultValue = "true") boolean createParentDirectories,
      MuleEvent event) {
    fileSystem.changeToBaseDir(config);
    validateTargetPath(targetPath);
   ...
   * {@link FileAttributes#getPath()} will be used. If that's not the case, then an {@link IllegalArgumentException} will be thrown.
   * <p>
   * {@code to} argument should not contain any path separator. {@link IllegalArgumentException} will be thrown if this precondition is not
   * honored.
   *
   * @param config the config that is parameterizing this operation
   * @param fileSystem a reference to the host {@link FileSystem}
  ...
  private String resolvePath(String path, MuleEvent event, String attributeName) {
    ...
    throw new IllegalArgumentException(
        format("A %s was not specified and a default one could not be obtained from the current message attributes", attributeName));
  }
