 ...
 * <p/>
 * The Mule implementation the <i>Enrichment Resource</i> can be any Message Processor. This allows you to not only use a JDBC endpoint
 * directly but also call out to a remote service via HTTP or even reference another flow or sub-flow.
 * <p/>
 * The Message Processor that implements the <i>Enrichment Resource</i> is invoked with a copy of the current message along with any flow or
 * session variables that are present. Invocation of the this message processor is done in a separate context to the main flow such that any
 * modification to the message (and it's properties and attachments) or flow or session variables will not be reflected in the flow where
 * the enricher is configured.
 * <p/>
 * The <i>Enrichment Resource</i> should always return a result. If it doesn't then the Enricher will simply leave the message untouched.
 * <p/>
 * The way in which the message is enriched (or modified) is by explicitly configuring mappings (source -> target) between the result from
 * the Enrichment Resource and the message using of Mule Expressions. Mule Expressions are used to both select the value to be extracted
 * from result that comes back from the enrichment resource (source) and to define where this value to be inserted into the message
 * (target). The default 'source' if it's not configured is the payload of the result from the enrichment resource..
 * <p/>
...
public class MessageEnricher extends AbstractMessageProcessorOwner implements NonBlockingMessageProcessor {
  ...
  /**
   * Enriches the current event using the result of processing the next message processor (the enrichment processor) and the configured
   * enrichment pairs.
   */
