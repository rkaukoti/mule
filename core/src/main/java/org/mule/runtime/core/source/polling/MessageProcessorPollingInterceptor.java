...
public abstract class MessageProcessorPollingInterceptor
{
    ...
    /**
     * Called after the polling message processor processes the event, but before an event is routed to the rest of the flow. As poll
     * creates an independent event if the polling message processor returns a message, any auxiliary information is discarded. This
     * extension point allows to carry over (or just add) properties to the event that will be sent to the rest of the flow. Note that the
     * source event is discarded after this point, so it doesn't make sense to modify it.
     *
