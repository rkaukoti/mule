...
/**
 * The default results handler for all outbound endpoint. Depending on the number of messages passed it the returning message will be
 * different. If the 'results' param is null or empty, null is returned. If the 'results' param contains a single
 * {@link org.mule.runtime.core.api.MuleMessage}, than that message is returned. If the 'results' param contains more than one message a
 * {@link org.mule.runtime.core.api.MuleMessageCollection} instance is returned.
 * <p/>
...
public class DefaultRouterResultsHandler implements RouterResultsHandler {
  ...
  /**
   * Aggregates the events in the results list into one single {@link org.mule.runtime.core.api.MuleEvent} You should only use this method
   * when you're sure that all the events in the results list were generated by the same thread that's going to handle the aggregated
   * response
   */
  @Override
  ...
  private MuleEvent createMessageCollectionWithSingleMessage(MuleEvent event) {
    final MuleMessage coll = MuleMessage.builder().collectionPayload(singletonList(event.getMessage()), MuleMessage.class).build();
