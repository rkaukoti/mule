...
public interface ValidationPhaseTemplate extends MessageProcessTemplate {
   ...
   *
   * In case that the message is not valid then {@link #discardInvalidMessage()} will be executed so the implementation can save the reason
   * why the message is invalid to report why the message has been discarded when {@link #discardInvalidMessage()} is called
   *
