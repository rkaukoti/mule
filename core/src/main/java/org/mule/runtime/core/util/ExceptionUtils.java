...
public class ExceptionUtils extends org.apache.commons.lang.exception.ExceptionUtils {
  ...
  /**
   * This method returns true if the throwable contains a {@link Throwable} that matches the specified class or subclass in the exception
   * chain. Subclasses of the specified class do match.
   *
  ...
  /**
   * This method returns the throwable closest to the root cause that matches the specified class or subclass. Any null argument will make
  ...
  /**
   * Similar to {@link #getFullStackTrace(Throwable)} but removing the exception and causes messages. This is useful to determine if two
   * exceptions have matching stack traces regardless of the messages which may contain invokation specific data
   *
  ...
  /**
   * Introspects the {@link Throwable} parameter to obtain the first {@link Throwable} of type {@link ConnectionException} in the exception
   * chain.
   *
  ...
  /**
   * Introspects the {@link Throwable} parameter to obtain the first {@link Throwable} of type {@code throwableType} in the exception chain
   * and return the cause of it.
   *
   ...
   * {@link ConnectionException} the same value will be returned. If the throwable parameter has a cause of itself, then an empty value will
   * be returned.
   *
   * @param throwable the last throwable on the exception chain.
  ...
  /**
   * Executes the given {@code callable} knowing that it might throw an {@link Exception} of type {@code expectedExceptionType}. If that
   * happens, then it will re throw such exception.
   * <p>
   * If the {@code callable} throws a {@link RuntimeException} of a different type, then it is also re-thrown. Finally, if an exception of
   * any different type is thrown, then it is handled by delegating into the {@code exceptionHandler}, which might in turn also throw an
   * exception or handle it returning a value.
   *
