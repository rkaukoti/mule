...
/**
 * <code>FutureMessageResult</code> is an MuleMessage result of a remote invocation on a Mule Server. This object makes the result available
 * to the client code once the request has been processed. This execution happens asynchronously.
 */
// @ThreadSafe
public class FutureMessageResult extends FutureTask {
  /**
   * This is a simple default Executor for FutureMessageResults. Instead of spawning a Thread for each invocation it uses a single daemon
   * Thread with an unbounded queue, so "truly" concurrent operation of multiple Futures or otherwise customized execution behaviour
   * requires calling the {@link #setExecutor(Executor)} method and passing in a custom {@link Executor}. This is strongly recommended in
   * order to provide better control over concurrency, resource consumption and possible overruns.
   * <p>
   * Reasons for these defaults:
   * <ul>
   * <li>a daemon thread does not block the VM on shutdown; lifecycle control should be done elsewhere (e.g. the provider of the custom
   ...
   * <li>the unbounded queue is not optimal but probably harmless since e.g. a MuleClient would have to create a LOT of Futures for an OOM.
   * Cancelled/timed out invocations are GC'ed so the problem is rather unlikely to occur.
   * </ul>
   */
