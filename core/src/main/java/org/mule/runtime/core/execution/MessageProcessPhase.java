 ...
 *
 * The phase will be part of a chain of responsibility were the phase can define the end of the execution of the set of phases by calling: -
 * {@link PhaseResultNotifier#phaseConsumedMessage()} which indicates that the phase has consume the message and it should not be longer
 * processed - {@link PhaseResultNotifier#phaseFailure(Exception)} which indicates that there was a failure during message processing.
 *
 * Whenever a phase finish execution it must call {@link PhaseResultNotifier#phaseSuccessfully()} which will cause the next phase to be
 * executed.
 *
 * Optionally a {@link MessageProcessPhase} can implement {@link Comparable<MessageProcessPhase>} to define the order in which it must be
 * positioned in the {@link MessageProcessPhase} chain
 */
