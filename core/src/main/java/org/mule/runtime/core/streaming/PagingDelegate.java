...
/**
 * A PagingDelegate is a {@link Closeable} capable of consuming a data feed in pages. Implementing this class does not guarantee thread
 * safeness. Check each particular implementation for information about that
 *
 * @since 3.5.0
 */
public abstract class PagingDelegate<T> implements Closeable {
  ...
  /**
   * returns the total amount of items in the unpaged resultset. In some scenarios, it might not be possible/convenient to actually retrieve
   * this value. -1 is returned in such a case.
   */
