...
public final class SecurityUtils {
  ...
  /**
   * Returns the default security provider that should be used in scenarios where ONE provider must be explicitly given. It will get the
   * first registered provider in order of preference, unless a system variable is defined with a provider name.
   *
   * <p>
   * <b>Note:</b> Use this method as a last resort for cases were a library always requires you to provide one. JCE already provides an
   * excellent provider selection algorithm, and many operations will automatically choose the best provider if you don't force one in
   * particular
   * </p>
   */
  public static Provider getDefaultSecurityProvider() {
    ...
    if (provider == null) {
      throw new IllegalStateException(
          "Can't find a suitable security provider. " + (providerName == null ? "" : "Provider name " + providerName + " was not found."));
    }
    return provider;
  }
}
