 ...
 * conditions are met (according to the JSR-250 spec) <ol> <li>The method MUST NOT have any parameters except in the case of EJB
 * interceptors in which case it takes an InvocationContext object as defined by the EJB specification. Note that Mule is not an EJB
 * container so the EJB case is ignored in Mule.</li> <li>The return type of the method MUST be void.</li> <li>The method MUST NOT throw a
 * checked exception.</li> <li>The method on which PostConstruct is applied MAY be public, protected, package private or private.</li>
 * <li>The method MUST NOT be static except for the application client.</li> <li>The method MAY be final or non-final, except in the case of
 * EJBs where it MUST be non-final. Note that Mule is not an EJB container so the EJB case is ignored in Mule.</li> </ol>
 *
