...
/**
 * <code>Interceptor</code> is based on a similar concept of servlet filters and works much the same way. This method is more commonally
 * known as the interceptor pattern and it allows for pre and processing of invocations on the object being intercepted.
 */
public interface Interceptor extends InterceptingMessageProcessor {

  /**
   * Invoked when the component should be called. The implementation can call next.process(event) to call the component or the next filer in
   * the chain.
   *
