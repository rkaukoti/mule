...
/**
 * Abstract implementation of {@link AbstractFlowConstruct} that allows a list of {@link MessageProcessor}s that will be used to process
...
public abstract class AbstractPipeline extends AbstractFlowConstruct implements Pipeline {
   ...
   * {@link #configureMessageProcessors(org.mule.runtime.core.api.processor.MessageProcessorChainBuilder)} method but if you wish to use
   * another {@link MessageProcessorBuilder} or just a single {@link MessageProcessor} then this method can be overridden and return a
    ...
    builder.chain(new AbstractRequestResponseMessageProcessor() {
      ...
      protected MuleEvent processRequest(MuleEvent event) throws MuleException {
        muleContext.getNotificationManager()
            .fireNotification(new PipelineMessageNotification(AbstractPipeline.this, event, PipelineMessageNotification.PROCESS_START));
      ...
      protected void processFinally(MuleEvent event, MessagingException exception) {
        muleContext.getNotificationManager().fireNotification(
            new PipelineMessageNotification(AbstractPipeline.this, event, PipelineMessageNotification.PROCESS_COMPLETE, exception));
      }
    });
  }
  ...
  protected void configurePostProcessors(MessageProcessorChainBuilder builder) throws MuleException {
    builder.chain(new MessageProcessor() {
      ...
      public MuleEvent process(MuleEvent event) throws MuleException {
        muleContext.getNotificationManager()
            .fireNotification(new PipelineMessageNotification(AbstractPipeline.this, event, PipelineMessageNotification.PROCESS_END));
        return event;
      }
    });
  }
  ...
  public void validateConstruct() throws FlowConstructInvalidException {
    ...
    if (userConfiguredAsyncProcessingStrategy && (!(messageSource == null || isCompatibleWithAsync) || redeliveryHandlerConfigured)) {
      throw new FlowConstructInvalidException(CoreMessages.createStaticMessage(
          "One of the message sources configured on this Flow is not " + "compatible with an asynchronous processing strategy.  Either "
    ...
    if (processingStrategy instanceof NonBlockingProcessingStrategy && messageSource != null
      ...
      throw new FlowConstructInvalidException(CoreMessages
          .createStaticMessage("The non-blocking processing strategy currently only supports " + "non-blocking messages sources"), this);
    }
