...
/**
 * A {@link ConnectionProviderWrapper} which decorates the {@link #delegate} with a user configured {@link PoolingProfile} or the default
 * one if is was not supplied by the user.
 * <p/>
 * The purpose of this wrapper is having the {@link #getHandlingStrategy(ConnectionHandlingStrategyFactory)} method use the configured
 * {@link #poolingProfile} instead of the default included in the {@link #delegate}
 * <p/>
 *
 * @since 4.0
 */
public final class PoolingConnectionProviderWrapper<Connection> extends ConnectionProviderWrapper<Connection> {
   ...
   */
  public PoolingConnectionProviderWrapper(ConnectionProvider<Connection> delegate, PoolingProfile poolingProfile, boolean disableValidation,
