...
/**
 * A TransformResolver is used to find transformers that match a certain criteria in the registry. Implementations of this interface will
 * use some or all of the information passed in to discover a matching transformer.
 * <p/>
 * Any implementations of this interface must be registered with the registry before it will get picked up. Typically this is done using the
 * <code>registry-bootstrap.properties</code>.
 *
 * @since 3.0.0
 */
public interface TransformerResolver {
  /**
   * Responsible for finding a transformer with the given criteria. Note that if a transformer is not found null should be return, an
  ...
  /**
   * A callback that is called when a transformer is registered or unregistered from the registry. This is used in situations where the
   * resolver caches transformers and the cache needs to be updated.
   *
  ...
  /**
   * Possible registry actions that occur that will trigger an event fired via {@link #transformerChange(Transformer, RegistryAction)}
   * method.
   */
  enum RegistryAction {
    /**
