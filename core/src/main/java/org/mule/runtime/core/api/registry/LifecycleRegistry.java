...
/**
 * A {@link Registry} which not only registers and fetches objects, but it's also capable of applying lifecycle and injects dependencies
 *
 * @since 3.7.0
 */
public interface LifecycleRegistry extends Registry {
  ...
  /**
   * Will fire the given lifecycle {@code phase} without actually registering the object in the registry. This is useful for prototype
   * objects that are created per request and would clutter the registry with single use objects.
   *
   * @param object the object to process
  ...
  /**
   * Applies lifecycle phase to an object independent of the current lifecycle phase. All phases between the {@code startPhase} and the
   * {@code endPhase} will be executed.
   *
   ...
   * <p/>
   * Because {@link #lookupObject(String)} will return objects which had lifecycle phases applied,this method exists for cases in which you
   * want to specify that lifecycle is not to be applied. The actual semantics of that actually depends on the implementation, since
   * invoking this method might return a new instance or an already existing one. If an existing one is returned, then the lifecycle might
