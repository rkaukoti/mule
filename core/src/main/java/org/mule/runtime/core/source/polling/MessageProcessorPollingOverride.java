 ...
 *
 * Implementation registered with Poll will be called on each execution to provide an interceptor which will then be called on several
 * execution checkpoints. Note that while this class should be threadsafe, interceptors returned will be scoped on a single execution, and
 * hence may carry state.
 */
public abstract class MessageProcessorPollingOverride {

  /**
   * Returns an interceptor instance. This method will be called on every poll, and may return a new instance every time or always the same
   * instance (the latter case requires the instance to be threadsafe). Interceptor's are scoped for each poll and flow execution and will
   * be discarded after the scope ends.
   *
