 ...
 * <p>
 * {@code this} instance is created associated to a specific instance which is returned on every invokation to {@link #getConnection()}.
 * Calling {@link #release()} or {@link #close()} means that {@link ConnectionProvider#disconnect(Object)} will be invoked on
 * {@link #connectionProvider} with {@link #connection} as argument.
 *
...
final class PassThroughConnectionHandler<Connection> implements ConnectionHandlerAdapter<Connection> {
  ...
  /**
   * Invokes {@link ConnectionProvider#disconnect(Object)} on {@link #connectionProvider} using {@link #connection} as an argument. This
   * method is thread safe, {@link #connection} will not be disconnected twice
   */
  @Override
  public void release() {
