...
/**
 * <code>IdempotentSecureHashMessageFilter</code> ensures that only unique messages are received by a service. It does this by calculating
 * the SHA-256 hash of the message itself. This provides a value with an infinitesimally small chance of a collision. This can be used to
 * filter message duplicates. Please keep in mind that the hash is calculated over the entire byte array representing the message, so any
 * leading or trailing spaces or extraneous bytes (like padding) can produce different hash values for the same semantic message content.
 * Care should be taken to ensure that messages do not contain extraneous bytes. This class is useful when the message does not support
 * unique identifiers.
 */
