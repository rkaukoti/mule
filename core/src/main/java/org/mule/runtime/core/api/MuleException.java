...
/**
 * <code>MuleException</code> is the base exception type for the Mule server any other exceptions thrown by Mule code will be based on this
 * exception,
 */
public abstract class MuleException extends Exception {
  ...
  public String getVerboseMessage() {
    ...
    for (Map.Entry entry : (Set<Map.Entry>) info.entrySet()) {
      ...
      buf.append((entry.getValue() == null ? "null"
          : entry.getValue().toString().replaceAll(SystemUtils.LINE_SEPARATOR, SystemUtils.LINE_SEPARATOR + StringUtils.repeat(' ', 24))))
  ...
  public String getSummaryMessage() {
    ...
    buf.append(SystemUtils.LINE_SEPARATOR).append(EXCEPTION_MESSAGE_DELIMITER);
    buf.append("Message               : ").append(message).append(SystemUtils.LINE_SEPARATOR);
    buf.append("Element               : ").append(ExceptionHelper.getExceptionInfo(this).get(LocatedMuleException.INFO_LOCATION_KEY))
        .append(SystemUtils.LINE_SEPARATOR);
    ...
    buf.append(SystemUtils.LINE_SEPARATOR).append("  (set debug level logging or '-Dmule.verbose.exceptions=true' for everything)")
        .append(SystemUtils.LINE_SEPARATOR);
    buf.append(EXCEPTION_MESSAGE_DELIMITER);
