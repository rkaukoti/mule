...
public interface ExpressionManager {
  ...
  /**
   * Evaluates the given expression. The expression should be a single expression definition with or without enclosing braces. i.e.
   * "context:serviceName" and "#[context:serviceName]" are both valid. For situations where one or more expressions need to be parsed
   * within a single text, the {@link #parse(String, org.mule.runtime.core.api.MuleEvent, boolean)} method should be used since it will
   * iterate through all expressions in a string.
   *
   ...
   * @param expression a single expression i.e. xpath://foo
   * @param event The current event being processed
   * @param failIfNull determines if an exception should be thrown if expression could not be evaluated or returns null. @return the result
   *        of the evaluation
  ...
  /**
   * Evaluates the given expression resolving the result of the evaluation to a boolean. The expression should be a single expression
   * definition with or without enclosing braces. i.e. "context:serviceName" and "#[context:serviceName]" are both valid.
   *
  ...
  /**
   * Evaluates expressions in a given string. This method will iterate through each expression and evaluate it. If a user needs to evaluate
   * a single expression they can use {@link #evaluate(String, org.mule.runtime.core.api.MuleEvent, boolean)}.
   *
   * @param expression one or more expressions ebedded in a literal string i.e. "Value is #[xpath://foo] other value is #[header:foo]."
   * @param event The current event being processed
   * @return the result of the evaluation
   * @throws org.mule.runtime.core.api.expression.ExpressionRuntimeException if the expression is invalid, or a null is found for the
  ...
  /**
   * Determines if the expression is valid or not. This method will validate a single expression or expressions embedded in a string. The
   * expression must either be a well formed expression evaluator i.e. #[bean:user] or must be a valid expression language expression.
   *
  ...
  /**
   * Determines if the string is an expression. This method will validate that the string contains either the expression prefix (malformed)
   * or a full expression. This isn't full proof but catches most error cases
   *
