 ...
 * <code>JavaComponent</code> uses an {@link ObjectFactory} to specify the object instance's source and allows for singleton and prototype
 * implementations to be used along with other custom {@link ObjectFactory} that allow component instances to be obtained from containers
 * such as Spring.
 */
public interface JavaComponent extends Component {

  /**
   * A {@link JavaComponent} can have a custom entry-point resolver for its own object. By default this is null. When set this resolver will
   * override the resolver on the model
   *
  ...
  /**
   * The object factory used to obtain the component object instance. Mule core provides two implementations: {@link SingletonObjectFactory}
   * and {@link PrototypeObjectFactory}.<br/>
   * The spring-config module provides an {@link ObjectFactory} implementation that delegates to spring. There is no PooledObjectFactory,
   * the {@link PooledJavaComponent} should be used for pooling.
   */
