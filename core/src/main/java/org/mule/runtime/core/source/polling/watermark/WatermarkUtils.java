...
public abstract class WatermarkUtils {
  ...
  /**
   * Evaluates a mel expression. If the value is not an expression or if it is not valid then returns the same value. The expression is
   * expected to
   *
    ...
    if (expressionManager.isExpression(expression) && expressionManager.isValidExpression(expression)) {
      ...
      if (evaluated != null && !(evaluated instanceof Serializable)) {
        throw new NotSerializableException(
            String.format("Expression %s resolves to an object that is not serializable (%s). It can't be used as watermark.", expression,
                evaluated.getClass().getCanonicalName()));
      }
