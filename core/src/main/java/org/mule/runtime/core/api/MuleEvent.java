 ...
 * <p/>
 * Holds a MuleMessage payload and provides helper methods for obtaining the data in a format that the receiving Mule component understands.
 * The event can also maintain any number of properties that can be set and retrieved by Mule components.
 *
...
public interface MuleEvent extends org.mule.runtime.api.message.MuleEvent {
  ...
  /**
   * Transforms the message into the requested format. The transformer used is the one configured on the endpoint through which this event
   * was received.
   *
   ...
   * configured on the endpoint through which this event was received. If necessary this will use the encoding set on the event
   *
  ...
  /**
   * Determines whether the default processing for this event will be executed. By default, the Mule server will route events according to a
   * components configuration. The user can override this behaviour by obtaining a reference to the MuleEvent context, either by
   * implementing <code>org.mule.runtime.core.api.lifecycle.Callable</code> or calling <code>RequestContext.getEventContext</code> to obtain
   * the MuleEventContext for the current thread. The user can programmatically control how events are dispatched.
   *
   ...
   * components configuration. The user can override this behaviour by obtaining a reference to the MuleEvent context, either by
   * implementing <code>org.mule.runtime.core.api.lifecycle.Callable</code> or calling <code>RequestContext.getEventContext</code> to obtain
   * the MuleEventContext for the current thread. The user can programmatically control how events are dispached.
   *
  ...
  /**
   * The number of milliseconds to wait for a return event when running synchronously. 0 wait forever -1 try and receive, but do not wait or
  ...
  /**
   * Events have a stack of executed flows (same as a call stack), so that at any given instant an application developer can determine where
   * this event came from.
   * <p/>
   * This will only be enabled if {@link DefaultMuleConfiguration#isFlowTrace()} is {@code true}. If {@code false}, the stack will always be
   * empty.
   *
  ...
  /**
   * Events have a list of message processor paths it went trough so that the execution path of an event can be reconstructed after it has
   * executed.
   * <p/>
   * This will only be enabled if {@link DefaultMuleConfiguration#isFlowTrace()} is {@code true}. If {@code false}, the list will always be
   * empty.
   *
