...
public class XaTransaction extends AbstractTransaction {
  ...
  protected synchronized void doCommit() throws TransactionException {
    try {
       ...
       * 
       * 3.2.2 Completing a Transaction The TransactionManager.commit method completes the transaction currently associated with the calling
       * thread.
       ****
       * 
       ...
       ****
       * 
       * If the commit method is called when the thread is not associated with any transaction context, the TM throws an exception. In some
       * implementations, the commit operation is restricted to the transaction originator only. If the calling thread is not allowed to
       * commit the transaction, the TM throws an exception. The TransactionManager.rollback method rolls back the transaction associated
       ...
       * 
       * 3.3.3 Transaction Completion The Transaction.commit and Transaction.rollback methods allow the target object to be comitted or
       * rolled back. The calling thread is not required to have the same transaction associated with the thread. If the calling thread is
       * not allowed to commit the transaction, the transaction manager throws an exception.
       * 
       * 
       * So what it meant was that one can't use Transaction.commit()/rollback(), as it doesn't properly disassociate the thread of
       * execution from the current transaction. There's no JTA API-way to do that after the call, so the thread's transaction is subject to
       * manual recovery process. Instead TransactionManager or UserTransaction must be used.
       */
      delistResources();
      txManager.commit();
      ...
      /*
       * MUST nullify XA ref here, otherwise Transaction.getStatus() doesn't match javax.transaction.Transaction.getStatus(). Must return
       * STATUS_NO_TRANSACTION and not STATUS_COMMITTED.
       * 
  ...
  /**
   * @param key Must be the provider of the resource object. i.e. for JDBC it's the XADataSource, for JMS is the XAConnectionFactory. It can
   *        be a wrapper in which case should be a {@link org.mule.runtime.core.util.xa.XaResourceFactoryHolder} to be able to determine
  ...
  private ResourceKey getResourceEntry(Object resourceFactory) {
    resourceFactory = (resourceFactory instanceof XaResourceFactoryHolder ? ((XaResourceFactoryHolder) resourceFactory).getHoldObject()
        : resourceFactory);
  ...
  /**
   * This class is used as key for the resources map since allows us to overcome some bad hashcode implementation of resource factories such
   * as org.enhydra.jdbc.standard.StandardDataSource.
   */
