...
/**
 * Immutable object used to provide all the necessary information to perform an aggregation operation in one single parameter, helping to
 * maintain consistent and simple signatures across aggregators The most important attribute in this class is {@link #events} which holds
 * the events to be aggregated. These events need to be ordered so that each event's index corresponds to the index of each route
 *
 * @since 3.5.0
 */
public final class AggregationContext {
   ...
   * @param events a {@link List} of {@link MuleEvent}. Cannot be <code>null</code> but could be empty. In that case, is up to each consumer
  ...
  /**
   * Returns all the {@link MuleEvent}s which messages have a not <code>null</code> {@link ExceptionPayload} and a not <code>null</code>
   * {@link ExceptionPayload#getException()}. Notice that this is a select operation. Each time this method is invoked the result will be
   * re-calculated
   *
  ...
  /**
   * Returns a {@link NavigableMap} in which the key is a zero-based route index and the value is an {@link Throwable} generated by it.
   * Notice that this is a collect operation. Each time this method is invoked the result will be re-calculated
   *
  ...
  /**
   * The exact opposite to {@link #collectEventsWithExceptions()} Returns all the {@link MuleEvent}s which messages have a <code>null</code>
   * {@link ExceptionPayload} or a <code>null</code> {@link ExceptionPayload#getException()}. Notice that this is a collect operation. Each
   * time this method is invoked the result will be re-calculated
   *
   * @return a list of {@link MuleEvent}. It could be empty but it will never be <code>null</code>
   */
  @SuppressWarnings("unchecked")
