...
/**
 * ExecutionTemplate created by this method should be used on the beginning of the execution of a chain of MessageProcessor that should
 * manage exceptions. Should be used when: An asynchronous MessageProcessor chain is being executed Because of an <async> element Because of
 * an asynchronous processing strategy A Flow is called using a <flow-ref> element
 *
...
public class ErrorHandlingExecutionTemplate implements ExecutionTemplate<MuleEvent> {
  ...
  /**
   * Creates a ErrorHandlingExecutionTemplate to be used as the main enthat will route any MessagingException thrown to an exception
   * listener
   *
