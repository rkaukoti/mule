...
public final class TransitiveDependenciesFilter {
  ...
  /**
   * Creates a new instance, only visible to package scope so the {@link Configuration} is the only one that should call this constructor.
   */
   ...
   * transitive dependencies will not be evaluated. It will stop traversing the tree at this point and continue with others , it should not
   * be included but it will continue with other leafs.
   * <p/>
   * By setting this that behaviour will change and instead of stopping traversing to its transitive dependencies, the resolution process
   * will not include the not matching transitive dependency but it will continue evaluating its transitive dependencies, if any of them
   * match the criteria they will be included and if not it wll continue with its transitive dependencies and so on.
   *
   * @return this
   */
