...
/**
 * Responsible of filtering, traversing and selectDependencies a list of URLs with different conditions and patterns in order to build class
 * loaders by filtering an initial and complete classpath urls and using a maven dependency graph represented by their dependencies
 * transitions in a {@link java.util.Map}.
 *
 * @since 4.0
 */
public class DependencyResolver {
   ...
   * @param predicate a filter to be applied for each transitive dependency, if the filter passes the dependency is added and recursively
   *        collected its dependencies using the same filter.
   ...
   */
  private Set<MavenArtifact> collectTransitiveDependencies(final MavenArtifact dependency, final Predicate<MavenArtifact> predicate,
    ...
    configuration.getDependencyGraph().getTransitiveDependencies(dependency).stream().forEach(transitiveDependency -> {
      if (predicate.test(transitiveDependency)) {
        ...
        // Just the case for getting all their dependencies from an excluded dependencies (case of org.mule:core for instance, we also need
