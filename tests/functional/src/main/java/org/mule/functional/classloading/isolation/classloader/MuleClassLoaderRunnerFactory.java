 ...
 * <p/>
 * The class loaders created have the following hierarchy: <ul> <li>Container: all the provided scope dependencies plus their dependencies
 * (non test dependencies) and java</li> <li>Plugins (optional): for each plugin a class loader will be created with all the compile scope
 * dependencies and their transitive dependencies (only the ones with scope compile)</li> <li>Application: all the test scope dependencies
 * and their dependencies if they are not defined to be excluded, plus their transitive dependencies (again if they are not excluded).</li>
 * </ul>
 *
 * @since 4.0
 */
public class MuleClassLoaderRunnerFactory
{
     ...
     *
     * @param extraBootPackages         {@link Set} of {@link String}s of extra boot packages to be appended to the container {@link
     *                                  ClassLoader}
     * @param artifactUrlClassification the {@link ArtifactUrlClassification} that defines the different {@link URL}s for each {@link
     *                                  ClassLoader}
     * @return a {@link ClassLoaderTestRunner} that would be used to run the test
     */
    public ClassLoaderTestRunner createClassLoaderTestRunner(Set<String> extraBootPackages,
     ...
     */
    private ArtifactClassLoader createContainerArtifactClassLoader(TestContainerClassLoaderFactory testContainerClassLoaderFactory,
                                                                   ArtifactUrlClassification artifactUrlClassification)
    {
        ...
        MuleArtifactClassLoader launcherArtifact = new MuleArtifactClassLoader("launcher", new URL[0],
                MuleClassLoaderRunnerFactory.class.getClassLoader(),
                new MuleClassLoaderLookupPolicy(Collections.emptyMap(), Collections.<String>emptySet()));
        ClassLoaderFilter filteredClassLoaderLauncher =
                new ContainerClassLoaderFilterFactory().create(testContainerClassLoaderFactory.getBootPackages(),
                        Collections.<MuleModule>emptyList());

        return testContainerClassLoaderFactory.createContainerClassLoader(
                new FilteringArtifactClassLoader(launcherArtifact, filteredClassLoaderLauncher));
    }
     ...
     * exported packages and resources for creating the filter from its mule-module.properties file. <pr/> It also creates a sharedLibs
     * plugin without any library so far, in order to be a similar representation as mule's container has and also to allow the hierarchy
     * process to look for classes from its parent (see {@link CompositeClassLoader} that doesn't delegate to its parent). <pr/> With the
     * given list of created class loader plugins it will finally create a {@link CompositeClassLoader} and return it.
     *
     ...
     * @param pluginsArtifactClassLoaders  a list where it would append each {@link ArtifactClassLoader} created for a plugin in order to
     *                                     allow access them later
                                                 ...
                                                 List<ArtifactClassLoader> pluginsArtifactClassLoaders)
    {
        ...
        for (PluginUrlClassification pluginUrlClassification : artifactUrlClassification.getPluginClassificationUrls())
        {
            ...
            else
            {
                ...
                List<MuleModule> modules = withContextClassLoader(pluginArtifactClassLoaderToDiscoverModules,
    ...
    /**
     * Validates that only one module should be discovered. A plugin cannot have inside another plugin that holds a {@code
     * mule-module.properties} for the time being.
     *
