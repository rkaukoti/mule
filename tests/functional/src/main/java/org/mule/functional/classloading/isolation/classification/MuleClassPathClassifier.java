...
/**
 * Builds a {@link ArtifactUrlClassification} similar to what Mule Runtime does by taking into account the Maven dependencies of the given
 * tested artifact.
 * <p/>
...
public class MuleClassPathClassifier implements ClassPathClassifier {
  ...
  public ArtifactUrlClassification classify(ClassPathClassifierContext context) {
    ...
    ExtendedClassPathClassifierContext extendedClassPathClassifierContext =
        new ExtendedClassPathClassifierContext(context, artifactToClassPathUrlResolver, targetTestClassesFolder);
  ...
  private List<URL> buildAppUrls(final ExtendedClassPathClassifierContext extendedContext) {
    // target/test-classes is not present in the dependency graph, so here is the only place were we must add it manually breaking the
    // original order that came from class path
    List<URL> appURLs = extendedContext.getClassificationContext().getClassPathURLs().stream()
        .filter(url -> url.getFile().equals(extendedContext.getTargetTestClassesFolder().getAbsolutePath() + File.separator))
        .collect(Collectors.toList());
    new DependencyResolver(new Configuration().setMavenDependencyGraph(extendedContext.getClassificationContext().getDependencyGraph())
        ...
        .collectTransitiveDependencies(new TransitiveDependenciesFilter()
            ...
            .evaluateTransitiveDependenciesWhenPredicateFails()))
                .resolveDependencies()
                .stream().filter(d -> !d.isPomType()).map(dependency -> extendedContext.getArtifactToClassPathURLResolver()
                    .resolveURL(dependency, extendedContext.getClassificationContext().getClassPathURLs()))
                .collect(Collectors.toCollection(() -> appURLs));
    return appURLs;
  }
  ...
  private List<PluginUrlClassification> buildPluginsUrlClassification(final ExtendedClassPathClassifierContext extendedContext) {
    ...
    String extensionsBasePackage = getAnnotationAttributeFrom(extendedContext.getClassificationContext().getTestClass(),
        ArtifactClassLoaderRunnerConfig.class, "extensionBasePackage");
    if (extensionsBasePackage != null && extensionsBasePackage.isEmpty()) {
      throw new IllegalArgumentException(
          "Base package for discovering extensions should be empty, it will take too much time to discover all the classes, please set a reasonable package for your extension annotate your tests with its base package");
    }
    ...
    if (!extensionsAnnotatedClasses.isEmpty()) {
      ...
      Set<String> extensionsAnnotatedClassesNoDups =
          extensionsAnnotatedClasses.stream().map(bd -> bd.getBeanClassName()).collect(Collectors.toSet());
      for (String extensionClassName : extensionsAnnotatedClassesNoDups) {
        ...
        String extensionMavenArtifactId =
            getExtensionMavenArtifactId(extensionClass, extendedContext.getClassificationContext().getMavenMultiModuleArtifactMapping());
    ...
    if (!isRootArtifactIdAnExtension
        && new File(extendedContext.getTargetTestClassesFolder().getParentFile(), CLASSES_FOLDER_NAME).exists()) {
      logger.debug(
  ...
  /**
   * Builds the list of {@link URL}s for the container classification. Provided scope is what mostly drives this classification minus
   * application {@link URL}s and plugins {@link URL}s.
   *
   * @param extendedContext {@link ExtendedClassPathClassifierContext} that holds the data needed for classifying the artifacts
   * @return a {@link List} of {@link URL}s that would be the one used for the container class loader.
   */
  private List<URL> buildContainerUrls(final ExtendedClassPathClassifierContext extendedContext, final List<URL> appURLs,
    ...
    // If a provided dependency was removed due to there is only one URL in class path for the same dependency, doesn't have the cardinality
    // that maven has
    new DependencyResolver(new Configuration().setMavenDependencyGraph(extendedContext.getClassificationContext().getDependencyGraph())
        ...
        .collectTransitiveDependencies(new TransitiveDependenciesFilter()
            ...
            .evaluateTransitiveDependenciesWhenPredicateFails())).resolveDependencies().stream().filter(d -> !d.isPomType())
                .map(dependency -> extendedContext.getArtifactToClassPathURLResolver().resolveURL(dependency,
  ...
  private String getExtensionMavenArtifactId(final Class extensionClass, final MavenMultiModuleArtifactMapping mavenMultiModuleMapping) {
    ...
    if (extensionSourceCodeLocation.isFile() && !relativeFolder.getName().equals(TARGET_FOLDER_NAME)) {
      ...
      extensionMavenArtifactId
          .append(mavenMultiModuleMapping.getArtifactId(relativeFolder.getParentFile().getAbsolutePath() + File.separator));
    }
  ...
  /**
   * It creates the resources for the given extension and does the classification of dependencies for the given extension and its artifactId
   * in order to collect the URLs to be used for the plugin {@link ClassLoader} for the extension.
   *
  ...
  private PluginUrlClassification extensionClassPathClassification(final Class extension, final String extensionMavenArtifactId,
    ...
    logger.debug("Extension classification for extension class : '{}', from artifactId: '{}'", extension.getName(),
        extensionMavenArtifactId);
    List<URL> extensionURLs = new ArrayList<>();

    // First we need to add META-INF folder for generated resources due to they may be already created by another mvn install goal by the
    // extension maven plugin
    File generatedResourcesDirectory = new File(extendedContext.getTargetTestClassesFolder().getParent(),
        GENERATED_TEST_SOURCES + File.separator + extensionMavenArtifactId + File.separator + "META-INF");
    ...
    new DependencyResolver(new Configuration().setMavenDependencyGraph(extendedContext.getClassificationContext().getDependencyGraph())
        ...
        .selectDependencies(
            new DependenciesFilter().match(dependency -> dependency.getArtifactId().equals(extensionMavenArtifactId.toString())
                || (extendedContext.getRootArtifact().getArtifactId().equals(extensionMavenArtifactId.toString())
                    && dependency.isCompileScope() && !extendedContext.getClassificationContext().getExclusions().test(dependency))))
        .collectTransitiveDependencies(
            new TransitiveDependenciesFilter().match(transitiveDependency -> transitiveDependency.isCompileScope()
                && !extendedContext.getClassificationContext().getExclusions().test(transitiveDependency)))).resolveDependencies().stream()
                    .filter(d -> !d.isPomType()).map(dependency -> extendedContext.getArtifactToClassPathURLResolver()
                        .resolveURL(dependency, extendedContext.getClassificationContext().getClassPathURLs()))
                    .forEach(extensionURLs::add);

    if (extensionURLs.size() == sizeBeforeDepResolver) {
      throw new IllegalStateException(
          "There should be at least one compile dependency found that matched to extension: '" + extension.getName()
  ...
  /**
   * Classifies URLs for a plugin that is not an extension where its compile dependencies and transitive dependencies should go to a plugin
   * {@link ClassLoader}.
   *
   * @param extendedContext {@link ExtendedClassPathClassifierContext} that holds the data needed for classifying the artifacts
  ...
  private PluginUrlClassification pluginClassPathClassification(final ExtendedClassPathClassifierContext extendedContext) {
    Set<URL> urls = new DependencyResolver(
        new Configuration().setMavenDependencyGraph(extendedContext.getClassificationContext().getDependencyGraph()).includeRootArtifact()
            .selectDependencies(new DependenciesFilter().match(
                dependency -> dependency.isCompileScope() && !extendedContext.getClassificationContext().getExclusions().test(dependency)))
            .collectTransitiveDependencies(
                new TransitiveDependenciesFilter().match(transitiveDependency -> transitiveDependency.isCompileScope()
