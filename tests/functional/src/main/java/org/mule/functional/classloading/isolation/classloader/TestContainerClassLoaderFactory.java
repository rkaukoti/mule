...
/**
 * Extends the default {@link ContainerClassLoaderFactory} for testing in order to add boot packages and build a {@link ClassLoader} for the
 * container that do resolves classes instead of delegating to its parent and also allows to create the container
 * {@link ClassLoaderLookupPolicy} based on a {@link ClassLoader}.
 *
 * @since 4.0
 */
public class TestContainerClassLoaderFactory extends ContainerClassLoaderFactory {
  ...
  /**
   * Factory class that extends the default way to create a container {@link ArtifactClassLoader} in order to support the differences when
   * running applications in standalone container vs junit.
   *
   * @param extraBootPackages {@link Set} of {@link String}s extra boot packages that need to be appended to the container (junit for
   *        instance)
  ...
  /**
   * Overrides the method in order to create a {@link ArtifactClassLoader} that will have a CHILD_FIRST {@link ClassLoaderLookupPolicy}, it
   * is needed due to as difference from a mule standalone container where the parent {@link ClassLoader} for the container only has
   * bootstrap jars plugins mule modules and third-party libraries when the runner runs this tests using has a full class path with all the
   * artifacts declared as dependencies for the artifact so we have to change that and change the look strategy to be CHILD_FIRST for the
   * container.
   *
   * @param parentClassLoader the parent {@link ClassLoader} to delegate PARENT look ups
   * @param muleModules {@link MuleModule} discovered from the launcher {@link ClassLoader} but will be not considered here due to it has
   *        all the class path
   * @param containerLookupPolicy the default {@link ClassLoaderLookupPolicy} defined for a container but will be ignored due to it has to
  ...
  /**
   * @return the original list of boot packages defined in {@link ContainerClassLoaderFactory} plus extra packages needed to be added in
   *         order to allow tests to run with isolation. For instance, junit is an extra package that has to be handled as boot package.
   */
  @Override
