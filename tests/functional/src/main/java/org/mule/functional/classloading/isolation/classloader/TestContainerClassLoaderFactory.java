...
public class TestContainerClassLoaderFactory extends ContainerClassLoaderFactory
{
    ...
    /**
     * Overrides the method in order to create a {@link ArtifactClassLoader} that will have a CHILD_FIRST {@link ClassLoaderLookupPolicy},
     * it is needed due to as difference from a mule standalone container where the parent {@link ClassLoader} for the container only has
     * bootstrap jars plugins mule modules and third-party libraries when the runner runs this tests using has a full class path with all
     * the artifacts declared as dependencies for the artifact so we have to change that and change the look strategy to be CHILD_FIRST for
     * the container.
     *
     * @param parentClassLoader     the parent {@link ClassLoader} to delegate PARENT look ups
     * @param muleModules           {@link MuleModule} discovered from the launcher {@link ClassLoader} but will be not considered here due
     *                              to it has all the class path
